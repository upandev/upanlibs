...
#define PNG_GET_CHUNK_BOILERPLATE(chunk) \
    ...
    int ret = ctx->read_chunks(0); \
    if(ret) return ret; \
    if(!ctx->_stored.chunk) return PNG_ECHUNKAVAIL; \
    if(chunk == NULL) return 1

#define PNG_SET_CHUNK_BOILERPLATE(chunk) \
    if(ctx == NULL || chunk == NULL) return 1; \
    if(ctx->_data == NULL && !ctx->_encode_only) return PNG_ENOSRC; \
    int ret = ctx->read_chunks(0); \
    if(ret) return ret
    ...
    static void* spng__zalloc(void *opaque, uint32_t items, uint32_t size) {
...
/* Returns the current sample unpacked, iterates to the next one */
    ...
    static unsigned num_channels(const struct iHeader *ihdr) {
...
/* Calculate scanline width in bits, round up to the nearest byte */
    static int calculate_scanline_width(const struct iHeader *ihdr, uint32_t width, size_t *scanline_width) {
      if (ihdr == NULL || !width) return PNG_EINTERNAL;
    ...
    int Context::calculate_subimages() {
      struct iHeader *ihdr = &_ihdr;
      struct spng_subimage *sub = _subimage;
    ...
    static int check_decode_fmt(const struct iHeader *ihdr, const int fmt) {
      switch (fmt) {
        case PNG_FMT_RGBA8:
        case PNG_FMT_RGBA16:
        case PNG_FMT_RGB8:
    ...
    static int calculate_image_width(const struct iHeader *ihdr, int fmt, size_t *len) {
      if (ihdr == NULL || len == NULL) return PNG_EINTERNAL;
    ...
    static int calculate_image_size(const struct iHeader *ihdr, int fmt, size_t *len) {
      if (ihdr == NULL || len == NULL) return PNG_EINTERNAL;
      ...
      if (res > SIZE_MAX / ihdr->height) return PNG_EOVERFLOW;
      printf("\nW:%u, H:%u", res, ihdr->height);
      res = res * ihdr->height;
    ...
    static int increase_cache_usage(Context *ctx, size_t bytes, int new_chunk) {
      if (ctx == NULL || !bytes) return PNG_EINTERNAL;

      if (new_chunk) {
        ctx->_chunk_count_total++;
        if (ctx->_chunk_count_total < 1) return PNG_EOVERFLOW;

        if (ctx->_chunk_count_total > ctx->_chunk_count_limit) return PNG_ECHUNK_LIMITS;
      }

      size_t new_usage = ctx->_chunk_cache_usage + bytes;

      if (new_usage < ctx->_chunk_cache_usage) return PNG_EOVERFLOW;

      if (new_usage > ctx->_chunk_cache_limit) return PNG_ECHUNK_LIMITS;

      ctx->_chunk_cache_usage = new_usage;

      return 0;
    }

    static int decrease_cache_usage(Context *ctx, size_t usage) {
      if (ctx == NULL || !usage) return PNG_EINTERNAL;
      if (usage > ctx->_chunk_cache_usage) return PNG_EINTERNAL;

      ctx->_chunk_cache_usage -= usage;

      return 0;
    }

    static int is_critical_chunk(struct Chunk *chunk) {
    ...
    int Context::on_error(int err) {
      _state = PNG_STATE_INVALID;
      return err;
    }

    int Context::read_data(size_t bytes) {
      if (!bytes) return 0;
...
/* Ensure there is enough space for encoding starting at ctx->_write_ptr  */
    int Context::require_bytes(size_t bytes) {
      if (_streaming) {
        if (bytes > _stream_buf_size) {
          ...
          if (temp == NULL) return on_error(PNG_EMEM);
    ...
    int Context::write_data(const void *data, size_t bytes) {
      if (!bytes) return 0;
    ...
    int Context::write_header(const uint8_t chunk_type[4], size_t chunk_length, unsigned char **data) {
      if (chunk_type == NULL) return PNG_EINTERNAL;
      if (chunk_length > spng_u32max) return PNG_EINTERNAL;
      ...
      uint32_t crc = gccucore::zlib::crc32(0, NULL, 0);
    ...
    int Context::trim_chunk(uint32_t length) {
      if (length > spng_u32max) return PNG_EINTERNAL;
    ...
    int Context::finish_chunk() {
      struct Chunk *chunk = &_current_chunk;
    ...
    int Context::write_chunk(const uint8_t type[4], const void *data, size_t length) {
      if (type == NULL) return PNG_EINTERNAL;
      if (length && data == NULL) return PNG_EINTERNAL;
...
/* Read and check the current chunk's crc,
   returns -PNG_CRC_DISCARD if the chunk should be discarded */
    int Context::read_and_check_crc() {
      int ret;
      ret = read_data(4);
      if (ret) return ret;
      ...
      if (_cur_actual_crc != _current_chunk.crc) {
        if (is_critical_chunk(&_current_chunk)) {
...
/* Read and validate the current chunk's crc and the next chunk header */
    int Context::read_header() {
      ...
      if (chunk.length > spng_u32max) return PNG_ECHUNK_STDLEN;

      _cur_chunk_bytes_left = chunk.length;

      if (is_critical_chunk(&chunk) && _crc_action_critical == PNG_CRC_USE) _skip_crc = 1;
      else if (_crc_action_ancillary == PNG_CRC_USE) _skip_crc = 1;
      else _skip_crc = 0;

      if (!_skip_crc) {
        _cur_actual_crc = gccucore::zlib::crc32(0, NULL, 0);
...
/* Read chunk bytes and update crc */
    int Context::read_chunk_bytes(uint32_t bytes) {
...
/* read_chunk_bytes() + read_data() with custom output buffer */
    static int read_chunk_bytes2(Context *ctx, void *out, uint32_t bytes) {
      if (ctx == NULL) return PNG_EINTERNAL;
      if (!ctx->_cur_chunk_bytes_left || !bytes) return PNG_EINTERNAL;
      if (bytes > ctx->_cur_chunk_bytes_left) return PNG_EINTERNAL; /* XXX: more specific error? */

      int ret;
      uint32_t len = bytes;

      if (ctx->_streaming && len > PNG_READ_SIZE) len = PNG_READ_SIZE;

      while (bytes) {
        if (len > bytes) len = bytes;

        ret = ctx->_read_fn(ctx, ctx->_stream_user_ptr, out, len);
        if (ret) return ret;

        if (!ctx->_streaming) memcpy(out, ctx->_data, len);

        ctx->_bytes_read += len;
        if (ctx->_bytes_read < len) return PNG_EOVERFLOW;

        if (!ctx->_skip_crc) ctx->_cur_actual_crc = gccucore::zlib::crc32(ctx->_cur_actual_crc, out, len);

        ctx->_cur_chunk_bytes_left -= len;
    ...
    int Context::discard_chunk_bytes(uint32_t bytes) {
      if (!bytes) return 0;
    ...
    static int spng__inflate_init(Context *ctx, int window_bits) {
      if (ctx->_zstream.state) gccucore::zlib::inflateEnd(&ctx->_zstream);

      ctx->_inflate = 1;

      ctx->_zstream.zalloc = spng__zalloc;
      ctx->_zstream.zfree = spng__zfree;
      ctx->_zstream.opaque = ctx;

      if (gccucore::zlib::inflateInit2(&ctx->_zstream, window_bits) != Z_OK) return PNG_EZLIB_INIT;

#if ZLIB_VERNUM >= 0x1290 && !defined(PNG_USE_MINIZ)

                                                                                                                              int validate = 1;

    if(ctx->_flags & PNG_CTX_IGNORE_ADLER32) validate = 0;

    if(is_critical_chunk(&ctx->_current_chunk))
    {
        if(ctx->_crc_action_critical == PNG_CRC_USE) validate = 0;
    }
    else /* ancillary */
    {
        if(ctx->_crc_action_ancillary == PNG_CRC_USE) validate = 0;
    }

    if(inflateValidate(&ctx->_zstream, validate)) return PNG_EZLIB_INIT;

#else /* This requires zlib >= 1.2.11 */
#pragma message ("inflateValidate() not available, PNG_CTX_IGNORE_ADLER32 will be ignored")
#endif

      return 0;
    }

    static int spng__deflate_init(Context *ctx, struct spng__zlib_options *options) {
      if (ctx->_zstream.state) gccucore::zlib::deflateEnd(&ctx->_zstream);

      ctx->_deflate = 1;

      gccucore::zlib::z_stream *zstream = &ctx->_zstream;
      zstream->zalloc = spng__zalloc;
      zstream->zfree = spng__zfree;
      zstream->opaque = ctx;
      zstream->data_type = options->data_type;

      int ret = gccucore::zlib::deflateInit2(zstream, options->compression_level, Z_DEFLATED, options->window_bits,
...
*/
    static int spng__inflate_stream(Context *ctx, char **out, size_t *len, size_t extra, const void *start_buf, size_t start_len) {
      int ret = spng__inflate_init(ctx, 15);
      if (ret) return ret;

      size_t max = ctx->_chunk_cache_limit - ctx->_chunk_cache_usage;

      if (ctx->_max_chunk_size < max) max = ctx->_max_chunk_size;
      ...
      if (buf == NULL) return PNG_EMEM;

      gccucore::zlib::z_stream *stream = &ctx->_zstream;
      ...
      while (ret != Z_STREAM_END) {
        ...
        {
          read_size = ctx->_cur_chunk_bytes_left;
          if (ctx->_streaming && read_size > PNG_READ_SIZE) read_size = PNG_READ_SIZE;

          ret = ctx->read_chunk_bytes(read_size);
...
/* Read at least one byte from the IDAT stream */
    int Context::read_idat_bytes(uint32_t *bytes_read) {
      if (bytes_read == NULL) return PNG_EINTERNAL;
    ...
    int Context::read_scanline_bytes(unsigned char *dest, size_t len) {
      if (dest == NULL) return PNG_EINTERNAL;
...
*/
    static int defilter_scanline(const unsigned char *prev_scanline, unsigned char *scanline,
      ...
      if (prev_scanline == NULL || scanline == NULL || !scanline_width) return PNG_EINTERNAL;
    ...
    static int
    filter_scanline(unsigned char *filtered, const unsigned char *prev_scanline, const unsigned char *scanline,
                    size_t scanline_width, unsigned bytes_per_pixel, const unsigned filter) {
      if (prev_scanline == NULL || scanline == NULL || scanline_width <= 1) return PNG_EINTERNAL;
...
/* Apply transparency to output row */
    ...
    static inline void
    scale_row(unsigned char *row, uint32_t pixels, int fmt, unsigned depth, const struct SBIT *sbit) {
      uint32_t i;
...
/* Unpack 1/2/4/8-bit samples to G8/GA8/GA16 or G16 -> GA16 */
    ...
    static int check_ihdr(const struct iHeader *ihdr, uint32_t max_width, uint32_t max_height) {
      if (ihdr->width > spng_u32max || !ihdr->width) return PNG_EWIDTH;
      if (ihdr->height > spng_u32max || !ihdr->height) return PNG_EHEIGHT;
    ...
    static int check_plte(const struct PLTE *plte, const struct iHeader *ihdr) {
    ...
    static int check_sbit(const struct SBIT *sbit, const struct iHeader *ihdr) {
    ...
    static int check_chrm_int(const struct ChrmInt *chrm_int) {
    ...
    static int check_phys(const struct PHYS *phys) {
      if (phys == NULL) return 1;

      if (phys->unit_specifier > 1) return PNG_EPHYS;

      if (phys->ppu_x > spng_u32max) return PNG_EPHYS;
      if (phys->ppu_y > spng_u32max) return PNG_EPHYS;

      return 0;
    }

    static int check_time(const struct Time *time) {
    ...
    static int check_offs(const struct Offs *offs) {
    ...
    static int check_exif(const struct Exif *exif) {
      if (exif == NULL) return 1;
      if (exif->data == NULL) return 1;

      if (exif->length < 4) return PNG_ECHUNK_SIZE;
      if (exif->length > spng_u32max) return PNG_ECHUNK_STDLEN;
...
/* Returns non-zero for standard chunks which are stored without allocating memory */
    ...
    int Context::read_ihdr() {
      ...
      ret = read_data(sizeof_sig_ihdr);
      if (ret) return ret;

      data = _data;

      if (memcmp(data, spng_signature, sizeof(spng_signature))) return PNG_ESIGNATURE;

      chunk->length = read_u32(data + 8);
      memcpy(&chunk->type, data + 12, 4);

      if (chunk->length != 13) return PNG_EIHDR_SIZE;
      if (memcmp(chunk->type, type_ihdr, 4)) return PNG_ENOIHDR;
      ...
      else _bytes_per_pixel = num_channels(&_ihdr) * (_ihdr.bit_depth / 8);
    ...
    static void splt_undo(Context *ctx) {
      struct SPLT *splt = &ctx->_splt_list[ctx->_n_splt - 1];

      free(splt->entries);

      decrease_cache_usage(ctx, sizeof(struct SPLT));
      decrease_cache_usage(ctx, splt->n_entries * sizeof(struct SPLTEntry));
    ...
    static void text_undo(Context *ctx) {
      struct spng_text2 *text = &ctx->_text_list[ctx->_n_text - 1];
    ...
    static void chunk_undo(Context *ctx) {
      struct UnknownChunk *chunk = &ctx->_chunk_list[ctx->_n_chunks - 1];

      free(chunk->data);

      decrease_cache_usage(ctx, chunk->length);
      decrease_cache_usage(ctx, sizeof(struct UnknownChunk));
    ...
    int Context::read_non_idat_chunks() {
      int ret;
      struct Chunk chunk;
      const unsigned char *data;
      ...
      while (!(ret = read_header())) {
        ...
        } else if (!memcmp(chunk.type, type_chrm, 4)) /* Ancillary chunks */
        {
          ...
          if (_gama > spng_u32max) return PNG_EGAMA;
        ...
        } else /* Arbitrary-length chunk */
        {
          ...
          if (!memcmp(chunk.type, type_exif, 4)) {
            ...
            if (increase_cache_usage(this, chunk.length, 1)) return PNG_ECHUNK_LIMITS;
            ...
            if (exif.data == NULL) return PNG_EMEM;

            ret = read_chunk_bytes2(this, exif.data, chunk.length);
            if (ret) {
              free(exif.data);
              return ret;
            }
            ...
            unsigned char *keyword_nul = memchr(_data, '\0', peek_bytes);
            if (keyword_nul == NULL) return PNG_EICCP_NAME;
            ...
            ret = spng__inflate_stream(this, &_iccp.profile, &_iccp.profile_len, 0, _data + keyword_len + 2,
            ...
            if (increase_cache_usage(this, sizeof(struct spng_text2), 1)) return PNG_ECHUNK_LIMITS;

            _n_text++;
            if (_n_text < 1) return PNG_EOVERFLOW;
            if (sizeof(struct spng_text2) > SIZE_MAX / _n_text) return PNG_EOVERFLOW;

            void *buf = realloc(_text_list, _n_text * sizeof(struct spng_text2));
            if (buf == NULL) return PNG_EMEM;
            _text_list = buf;

            struct spng_text2 *text = &_text_list[_n_text - 1];
            ...
            if (keyword_nul == NULL) return PNG_ETEXT_KEYWORD;
            ...
            if (!memcmp(chunk.type, type_text, 4)) {
              ...
              if (term == NULL) return PNG_EITXT_LANG_TAG;
              ...
              if (zlib_stream == NULL) return PNG_EITXT;
            ...
            if (text->compression_flag) {
              ...
              if (increase_cache_usage(this, peek_bytes, 0)) return PNG_ECHUNK_LIMITS;

              text->keyword = calloc(1, peek_bytes);
              if (text->keyword == NULL) return PNG_EMEM;

              memcpy(text->keyword, data, peek_bytes);

              zlib_stream = _data + text_offset;

              ret = spng__inflate_stream(this, &text->text, &text->text_length, 1, zlib_stream,
              ...
              if (increase_cache_usage(this, chunk.length + 1, 0)) return PNG_ECHUNK_LIMITS;

              text->keyword = malloc(chunk.length + 1);
              if (text->keyword == NULL) return PNG_EMEM;

              memcpy(text->keyword, data, peek_bytes);

              if (chunk.length > peek_bytes) {
                ret = read_chunk_bytes2(this, text->keyword + peek_bytes, chunk.length - peek_bytes);
                if (ret) return ret;
              }
            ...
            if (increase_cache_usage(this, chunk.length + sizeof(struct SPLT), 1)) return PNG_ECHUNK_LIMITS;
            ...
            if (increase_cache_usage(this, list_size, 0)) return PNG_ECHUNK_LIMITS;

            splt->entries = malloc(list_size);
            ...
            free(t);
            decrease_cache_usage(this, chunk.length);
          ...
          } else /* Unknown chunk */
          {
            ...
            if (increase_cache_usage(this, chunk.length + sizeof(struct UnknownChunk), 1))
...
/* Read chunks before or after the IDAT chunks depending on state */
    ...
    int Context::read_chunks(bool only_ihdr) {
      if (!_state) return PNG_EBADSTATE;
      if (_data == NULL) {
        if (_encode_only) return 0;
        else return PNG_EINTERNAL;
      }
      ...
      while (_state < PNG_STATE_FIRST_IDAT || _state == PNG_STATE_AFTER_IDAT) {
        ...
        if (!ret) {
          ...
          switch (ret) {
            ...
            case PNG_EEXIF:
            case PNG_EZLIB: {
              if (!_strict && !is_critical_chunk(&_current_chunk)) {
                ret = discard_chunk_bytes(_cur_chunk_bytes_left);
                if (ret) return on_error(ret);
    ...
    int Context::read_scanline() {
      int ret, pass = _row_info.pass;
      struct RowInfo *ri = &_row_info;
      const struct spng_subimage *sub = _subimage;
    ...
    int Context::spng_decode_scanline(void *out, size_t len) {
      if (out == NULL) return 1;

      if (_state >= PNG_STATE_EOI) return PNG_EOI;

      struct decode_flags f = _decode_flags;

      struct RowInfo *ri = &_row_info;
      const struct spng_subimage *sub = _subimage;

      const struct iHeader *ihdr = &_ihdr;
      ...
      struct spng__iter iter = spng__iter_init(ihdr->bit_depth, _scanline);
        ...
        if (ihdr->color_type == PNG_COLOR_TYPE_TRUECOLOR) {
          ...
          if (ihdr->bit_depth == 8) {
            if (fmt & (PNG_FMT_RGBA8 | PNG_FMT_RGB8)) {
              expand_row(out, scanline, &_decode_plte, width, fmt);
              break;
            }
      ...
      if (f.apply_trns) trns_row(out, scanline, trns_px, _bytes_per_pixel, &_ihdr, width, fmt);
    ...
    int Context::spng_decode_row(void *out, size_t len) {
      if (out == NULL) return 1;
      if (_state >= PNG_STATE_EOI) return PNG_EOI;
      ...
      if (!ihdr->interlace_method || pass == 6) return spng_decode_scanline(out, len);

      ret = spng_decode_scanline(_row, _image_width);
      ...
      if (_fmt == PNG_FMT_RGBA16) pixel_size = 8;
        ...
        if (ihdr->bit_depth < 8) {
          struct spng__iter iter = spng__iter_init(ihdr->bit_depth, _row);
    ...
    int Context::spng_decode_chunks() {
      if (_encode_only) return PNG_ECTXTYPE;
      if (_state < PNG_STATE_INPUT) return PNG_ENOSRC;
      if (_state == PNG_STATE_IEND) return 0;

      return read_chunks(0);
    }

    int Context::spng_decode_image(void *out, size_t len, int fmt, int flags) {
      if (_encode_only) return PNG_ECTXTYPE;
      if (_state >= PNG_STATE_EOI) return PNG_EOI;

      const struct iHeader *ihdr = &_ihdr;

      int ret = read_chunks(0);
      if (ret) return on_error(ret);

      ret = check_decode_fmt(ihdr, fmt);
      if (ret) return ret;

      ret = calculate_image_width(ihdr, fmt, &_image_width);
      if (ret) return on_error(ret);
      ...
      if (bytes_read > 1) {
        ...
        if (valid) _image_options.compression_level = compression_level;
      }

      ret = spng__inflate_init(this, _image_options.window_bits);
      if (ret) return on_error(ret);
      ...
      _scanline_buf = malloc(scanline_buf_size);
      _prev_scanline_buf = malloc(scanline_buf_size);
      ...
      if (ihdr->interlace_method) {
        f.interlaced = 1;
        _row_buf = malloc(_image_width);
        _row = _row_buf;

        if (_row == NULL) return on_error(PNG_EMEM);
      }

      if (_scanline == NULL || _prev_scanline == NULL) {
      ...
      if (f.apply_gamma) {
        ...
        } else /* PNG_FMT_RGBA16 */
        {
          ...
          _gamma_lut16 = malloc(lut_entries * sizeof(uint16_t));
          if (_gamma_lut16 == NULL) return on_error(PNG_EMEM);
    ...
    the last scanline will end up being 1 byte "shorter". */
      ...
      do {
        ...
        ret = spng_decode_row((unsigned char *) out + ioffset, _image_width);
      } while (!ret);
    ...
    int spng_get_row_info(Context *ctx, struct RowInfo *row_info) {
      if (ctx == NULL || row_info == NULL || ctx->_state < PNG_STATE_DECODE_INIT) return 1;

      if (ctx->_state >= PNG_STATE_EOI) return PNG_EOI;

      *row_info = ctx->_row_info;

      return 0;
    }

    int Context::write_chunks_before_idat() {
      ...
      ret = write_data(spng_signature, 8);
      if (ret) return ret;
      ...
      if (_stored.iccp) {
        ...
        byte *buf = malloc(dest_len);
        if (buf == NULL) return PNG_EMEM;

        ret = gccucore::zlib::compress2(buf, &dest_len, (void *) _iccp.profile,
                                        (unsigned long) _iccp.profile_len,
      ...
      if (_stored.text) {
        ...
        for (i = 0; i < _n_text; i++) {
          ...
          if (text->compression_flag) {
            ret = spng__deflate_init(this, &_text_options);
            if (ret) return ret;

            gccucore::zlib::z_stream *zstream = &_zstream;
            unsigned long dest_len = gccucore::zlib::deflateBound(zstream, (unsigned long) text_length);

            compressed_text = malloc(dest_len);

            if (compressed_text == NULL) return PNG_EMEM;
    ...
    int Context::write_chunks_after_idat() {
...
/* Compress and write scanline to IDAT stream */
    int Context::write_idat_bytes(const void *scanline, size_t len, int flush) {
      if (scanline == NULL) return PNG_EINTERNAL;
    ...
    int Context::encode_scanline(const void *scanline, size_t len) {
      if (scanline == NULL) return PNG_EINTERNAL;
      ...
      struct RowInfo *ri = &_row_info;
      const struct spng_subimage *sub = _subimage;
      struct encode_flags f = _encode_flags;
    ...
    int Context::encode_row(const void *row, size_t len) {
      if (row == NULL) return PNG_EINTERNAL;
    ...
    int Context::spng_encode_scanline(const void *scanline, size_t len) {
      if (scanline == NULL) return PNG_EINVAL;
      if (_state >= PNG_STATE_EOI) return PNG_EOI;
      if (len < (_subimage[_row_info.pass].scanline_width - 1)) return PNG_EBUFSIZ;

      return encode_scanline(scanline, len);
    }

    int Context::spng_encode_row(const void *row, size_t len) {
      if (row == NULL) return PNG_EINVAL;
      if (_state >= PNG_STATE_EOI) return PNG_EOI;
      if (len < _image_width) return PNG_EBUFSIZ;

      return encode_row(row, len);
    }

    int Context::spng_encode_chunks() {
      if (!_state) return PNG_EBADSTATE;
    ...
    int Context::spng_encode_image(const void *img, size_t len, int fmt, int flags) {
      if (!_state) return PNG_EBADSTATE;
      if (!_encode_only) return PNG_ECTXTYPE;
      if (!_stored.ihdr) return PNG_ENOIHDR;
      ...
      int ret = 0;
      const struct iHeader *ihdr = &_ihdr;
      struct encode_flags *encode_flags = &_encode_flags;
      ...
      else _bytes_per_pixel = num_channels(ihdr) * (ihdr->bit_depth / 8);

      if (_optimize_option & (1 << PNG_FILTER_CHOICE)) {
        /* Filtering would make no difference */
        if (!_image_options.compression_level) {
      ...
      if (!encode_flags->filter_choice && _optimize_option & (1 << PNG_IMG_COMPRESSION_STRATEGY)) {
        _image_options.strategy = Z_DEFAULT_STRATEGY;
      }

      ret = spng__deflate_init(this, &_image_options);
      if (ret) return on_error(ret);
      ...
      if (_scanline_buf == NULL || _prev_scanline_buf == NULL) return on_error(PNG_EMEM);
      ...
      if (encode_flags->filter_choice) {
        _filtered_scanline_buf = malloc(scanline_buf_size);
        if (_filtered_scanline_buf == NULL) return on_error(PNG_EMEM);
    ...
    Context::Context(int flags) {
      if (flags != (flags & PNG__CTX_FLAGS_ALL)) {
        throw upan::exception(XLOC, "invalid PNG context flags: %d", flags);
      }

      memset(this, 0, sizeof(Context));

      _max_width = spng_u32max;
      _max_height = spng_u32max;

      _max_chunk_size = spng_u32max;
      ...
      _image_options = image_defaults;
      _text_options = text_defaults;
      ...
      if (flags & PNG_CTX_ENCODER) _encode_only = true;
    }

    Context::~Context() {
      if (_streaming && _stream_buf != NULL) free(_stream_buf);
      ...
      if (_chunk_list != NULL && !_user.unknown) {
    ...
    static int buffer_read_fn(Context *ctx, void *user, void *data, size_t n) {
      if (n > ctx->_bytes_left) return PNG_IO_EOF;

      (void) user;
      (void) data;
      ctx->_data = ctx->_data + ctx->_last_read_size;

      ctx->_last_read_size = n;
      ctx->_bytes_left -= n;

      return 0;
    }

    static int file_read_fn(Context *ctx, void *user, void *data, size_t n) {
      FILE *file = user;
      (void) ctx;

      if (fread(data, n, 1, file) != 1) {
        if (feof(file)) return PNG_IO_EOF;
        else return PNG_IO_ERROR;
      }

      return 0;
    }

    static int file_write_fn(Context *ctx, void *user, void *data, size_t n) {
      FILE *file = user;
      (void) ctx;

      if (fwrite(data, n, 1, file) != 1) return PNG_IO_ERROR;
    ...
    int spng_set_png_buffer(Context *ctx, const void *buf, size_t size) {
      ...
      if (!ctx->_state) return PNG_EBADSTATE;
      if (ctx->_encode_only) return PNG_ECTXTYPE; /* not supported */

      if (ctx->_data != NULL) return PNG_EBUF_SET;

      ctx->_data = buf;
      ctx->_png_base = buf;
      ctx->_data_size = size;
      ctx->_bytes_left = size;

      ctx->_read_fn = buffer_read_fn;

      ctx->_state = PNG_STATE_INPUT;
    ...
    int spng_set_png_stream(Context *ctx, spng_rw_fn *rw_func, void *user) {
      ...
      if (ctx->_state >= PNG_STATE_INPUT) return PNG_EBUF_SET;

      if (ctx->_encode_only) {
        if (ctx->_out_png != NULL) return PNG_EBUF_SET;

        ctx->_write_fn = rw_func;
        ctx->_write_ptr = ctx->_stream_buf;

        ctx->_state = PNG_STATE_OUTPUT;
      } else {
        ctx->_stream_buf = malloc(PNG_READ_SIZE);
        if (ctx->_stream_buf == NULL) return PNG_EMEM;

        ctx->_read_fn = rw_func;
        ctx->_data = ctx->_stream_buf;
        ctx->_data_size = PNG_READ_SIZE;

        ctx->_state = PNG_STATE_INPUT;
      }

      ctx->_stream_user_ptr = user;
    ...
    int spng_set_png_file(Context *ctx, FILE *file) {
      ...
      if (ctx->_encode_only) return spng_set_png_stream(ctx, file_write_fn, file);
    ...
    void *spng_get_png_buffer(Context *ctx, size_t *len, int *error) {
      ...
      *error = 0;
      ...
      if (ctx == NULL || !len) *error = PNG_EINVAL;

      if (*error) return NULL;

      if (!ctx->_encode_only) *error = PNG_ECTXTYPE;
      else if (!ctx->_state) *error = PNG_EBADSTATE;
      else if (!ctx->_internal_buffer) *error = PNG_EOPSTATE;
      else if (ctx->_state < PNG_STATE_EOI) *error = PNG_EOPSTATE;
      else if (ctx->_state != PNG_STATE_IEND) *error = PNG_ENOTFINAL;

      if (*error) return NULL;

      ctx->_user_owns_out_png = 1;

      *len = ctx->_bytes_encoded;
    ...
    int spng_set_image_limits(Context *ctx, uint32_t width, uint32_t height) {
      ...
      ctx->_max_width = width;
      ctx->_max_height = height;
    ...
    int spng_get_image_limits(Context *ctx, uint32_t *width, uint32_t *height) {
      ...
      *width = ctx->_max_width;
      *height = ctx->_max_height;

      return 0;
    }

    int spng_set_chunk_limits(Context *ctx, size_t chunk_size, size_t cache_limit) {
      if (ctx == NULL || chunk_size > spng_u32max || chunk_size > cache_limit) return 1;

      ctx->_max_chunk_size = chunk_size;

      ctx->_chunk_cache_limit = cache_limit;

      return 0;
    }

    int spng_get_chunk_limits(Context *ctx, size_t *chunk_size, size_t *cache_limit) {
      if (ctx == NULL || chunk_size == NULL || cache_limit == NULL) return 1;

      *chunk_size = ctx->_max_chunk_size;

      *cache_limit = ctx->_chunk_cache_limit;
    ...
    int spng_set_crc_action(Context *ctx, int critical, int ancillary) {
      if (ctx == NULL) return 1;
      if (ctx->_encode_only) return PNG_ECTXTYPE;
      ...
      ctx->_crc_action_critical = critical;
      ctx->_crc_action_ancillary = ancillary;

      return 0;
    }

    int spng_set_option(Context *ctx, Option option, int value) {
      if (ctx == NULL) return 1;
      if (!ctx->_state) return PNG_EBADSTATE;

      switch (option) {
        case PNG_KEEP_UNKNOWN_CHUNKS: {
          ctx->_keep_unknown = value ? 1 : 0;
          break;
        }
        case PNG_IMG_COMPRESSION_LEVEL: {
          ctx->_image_options.compression_level = value;
          break;
        }
        case PNG_IMG_WINDOW_BITS: {
          ctx->_image_options.window_bits = value;
          break;
        }
        case PNG_IMG_MEM_LEVEL: {
          ctx->_image_options.mem_level = value;
          break;
        }
        case PNG_IMG_COMPRESSION_STRATEGY: {
          ctx->_image_options.strategy = value;
          break;
        }
        case PNG_TEXT_COMPRESSION_LEVEL: {
          ctx->_text_options.compression_level = value;
          break;
        }
        case PNG_TEXT_WINDOW_BITS: {
          ctx->_text_options.window_bits = value;
          break;
        }
        case PNG_TEXT_MEM_LEVEL: {
          ctx->_text_options.mem_level = value;
          break;
        }
        case PNG_TEXT_COMPRESSION_STRATEGY: {
          ctx->_text_options.strategy = value;
          break;
        }
        case PNG_FILTER_CHOICE: {
          if (value & ~PNG_FILTER_CHOICE_ALL) return 1;
          ctx->_encode_flags.filter_choice = value;
          break;
        }
        case PNG_CHUNK_COUNT_LIMIT: {
          if (value < 0) return 1;
          if (value > (int) ctx->_chunk_count_total) return 1;
          ctx->_chunk_count_limit = value;
          break;
        }
        case PNG_ENCODE_TO_BUFFER: {
          if (value < 0) return 1;
          if (!ctx->_encode_only) return PNG_ECTXTYPE;
          if (ctx->_state >= PNG_STATE_OUTPUT) return PNG_EOPSTATE;

          if (!value) break;

          ctx->_internal_buffer = 1;
          ctx->_state = PNG_STATE_OUTPUT;
      ...
      if (option < 32) ctx->_optimize_option &= ~(1 << option);

      return 0;
    }

    int spng_get_option(Context *ctx, Option option, int *value) {
      if (ctx == NULL || value == NULL) return 1;
      if (!ctx->_state) return PNG_EBADSTATE;

      switch (option) {
        case PNG_KEEP_UNKNOWN_CHUNKS: {
          *value = ctx->_keep_unknown;
          break;
        }
        case PNG_IMG_COMPRESSION_LEVEL: {
          *value = ctx->_image_options.compression_level;
          break;
        }
        case PNG_IMG_WINDOW_BITS: {
          *value = ctx->_image_options.window_bits;
          break;
        }
        case PNG_IMG_MEM_LEVEL: {
          *value = ctx->_image_options.mem_level;
          break;
        }
        case PNG_IMG_COMPRESSION_STRATEGY: {
          *value = ctx->_image_options.strategy;
          break;
        }
        case PNG_TEXT_COMPRESSION_LEVEL: {
          *value = ctx->_text_options.compression_level;
          break;
        }
        case PNG_TEXT_WINDOW_BITS: {
          *value = ctx->_text_options.window_bits;
          break;
        }
        case PNG_TEXT_MEM_LEVEL: {
          *value = ctx->_text_options.mem_level;
          break;
        }
        case PNG_TEXT_COMPRESSION_STRATEGY: {
          *value = ctx->_text_options.strategy;
          break;
        }
        case PNG_FILTER_CHOICE: {
          *value = ctx->_encode_flags.filter_choice;
          break;
        }
        case PNG_CHUNK_COUNT_LIMIT: {
          *value = ctx->_chunk_count_limit;
          break;
        }
        case PNG_ENCODE_TO_BUFFER: {
          if (ctx->_internal_buffer) *value = 1;
          else *value = 0;
    ...
    int Context::spng_decoded_image_size(int fmt, size_t *len) {
      if (len == NULL) return 1;

      int ret = read_chunks(1);
      if (ret) return ret;

      ret = check_decode_fmt(&_ihdr, fmt);
      if (ret) return ret;

      return calculate_image_size(&_ihdr, fmt, len);
    }

    int spng_get_ihdr(Context *ctx, struct iHeader *ihdr) {
      if (ctx == NULL) return 1;
      int ret = ctx->read_chunks(1);
      if (ret) return ret;
    ...
    int spng_get_chrm(Context *ctx, struct Chrm *chrm) {
      ...
      chrm->white_point_x = (double) ctx->_chrm_int.white_point_x / 100000.0;
      chrm->white_point_y = (double) ctx->_chrm_int.white_point_y / 100000.0;
      chrm->red_x = (double) ctx->_chrm_int.red_x / 100000.0;
      chrm->red_y = (double) ctx->_chrm_int.red_y / 100000.0;
      chrm->blue_y = (double) ctx->_chrm_int.blue_y / 100000.0;
      chrm->blue_x = (double) ctx->_chrm_int.blue_x / 100000.0;
      chrm->green_x = (double) ctx->_chrm_int.green_x / 100000.0;
      chrm->green_y = (double) ctx->_chrm_int.green_y / 100000.0;

      return 0;
    }

    int spng_get_chrm_int(Context *ctx, struct ChrmInt *chrm) {
      PNG_GET_CHUNK_BOILERPLATE(chrm);
    ...
    int spng_get_gama(Context *ctx, double *gamma) {
      ...
      *gama = (double) ctx->_gama / 100000.0;
    ...
    int spng_get_srgb(Context *ctx, uint8_t *rendering_intent) {
      ...
      *srgb = ctx->_srgb_rendering_intent;
    ...
    int spng_get_text(Context *ctx, struct TEXT *text, uint32_t *n_text) {
      ...
      int ret = ctx->read_chunks(0);
      if (ret) return ret;
      if (!ctx->_stored.text) return PNG_ECHUNKAVAIL;
      ...
      if (*n_text < ctx->_n_text) return 1;

      uint32_t i;
      for (i = 0; i < ctx->_n_text; i++) {
        text[i].type = ctx->_text_list[i].type;
        memcpy(&text[i].keyword, ctx->_text_list[i].keyword, strlen(ctx->_text_list[i].keyword) + 1);
        text[i].compression_method = 0;
        text[i].compression_flag = ctx->_text_list[i].compression_flag;
        text[i].language_tag = ctx->_text_list[i].language_tag;
        text[i].translated_keyword = ctx->_text_list[i].translated_keyword;
        text[i].length = ctx->_text_list[i].text_length;
        text[i].text = ctx->_text_list[i].text;
      }
    ...
    int spng_get_splt(Context *ctx, struct SPLT *splt, uint32_t *n_splt) {
      ...
      int ret = ctx->read_chunks(0);
      if (ret) return ret;
      if (!ctx->_stored.splt) return PNG_ECHUNKAVAIL;
      ...
      if (*n_splt < ctx->_n_splt) return 1;

      memcpy(splt, ctx->_splt_list, ctx->_n_splt * sizeof(struct SPLT));
    ...
    int spng_get_unknown_chunks(Context *ctx, struct UnknownChunk *chunks, uint32_t *n_chunks) {
      ...
      int ret = ctx->read_chunks(0);
      if (ret) return ret;
      if (!ctx->_stored.unknown) return PNG_ECHUNKAVAIL;
      if (n_chunks == NULL) return 1;

      if (chunks == NULL) {
        *n_chunks = ctx->_n_chunks;
        return 0;
      }

      if (*n_chunks < ctx->_n_chunks) return 1;

      memcpy(chunks, ctx->_chunk_list, sizeof(struct UnknownChunk));
    ...
    int spng_set_ihdr(Context *ctx, struct iHeader *ihdr) {
      PNG_SET_CHUNK_BOILERPLATE(ihdr);

      if (ctx->_stored.ihdr) return 1;

      ret = check_ihdr(ihdr, ctx->_max_width, ctx->_max_height);
      if (ret) return ret;
    ...
    int spng_set_plte(Context *ctx, struct PLTE *plte) {
      PNG_SET_CHUNK_BOILERPLATE(plte);

      if (!ctx->_stored.ihdr) return 1;

      if (check_plte(plte, &ctx->_ihdr)) return 1;

      ctx->_plte.n_entries = plte->n_entries;

      memcpy(ctx->_plte.entries, plte->entries, plte->n_entries * sizeof(struct PLTEEntry));
    ...
    int spng_set_trns(Context *ctx, struct Trns *trns) {
      PNG_SET_CHUNK_BOILERPLATE(trns);

      if (!ctx->_stored.ihdr) return PNG_ENOIHDR;

      if (ctx->_ihdr.color_type == PNG_COLOR_TYPE_GRAYSCALE) {
        ctx->_trns.gray = trns->gray;
      } else if (ctx->_ihdr.color_type == PNG_COLOR_TYPE_TRUECOLOR) {
        ctx->_trns.red = trns->red;
        ctx->_trns.green = trns->green;
        ctx->_trns.blue = trns->blue;
      } else if (ctx->_ihdr.color_type == PNG_COLOR_TYPE_INDEXED) {
        if (!ctx->_stored.plte) return PNG_ETRNS_NO_PLTE;
        if (trns->n_type3_entries > ctx->_plte.n_entries) return 1;

        ctx->_trns.n_type3_entries = trns->n_type3_entries;
        memcpy(ctx->_trns.type3_alpha, trns->type3_alpha, trns->n_type3_entries);
    ...
    int spng_set_chrm(Context *ctx, struct Chrm *chrm) {
      PNG_SET_CHUNK_BOILERPLATE(chrm);
    ...
    int spng_set_chrm_int(Context *ctx, struct ChrmInt *chrm_int) {
      PNG_SET_CHUNK_BOILERPLATE(chrm_int);
    ...
    int spng_set_iccp(Context *ctx, struct ICCP *iccp) {
      PNG_SET_CHUNK_BOILERPLATE(iccp);

      if (check_png_keyword(iccp->profile_name)) return PNG_EICCP_NAME;
      if (!iccp->profile_len) return PNG_ECHUNK_SIZE;
      if (iccp->profile_len > spng_u32max) return PNG_ECHUNK_STDLEN;

      if (ctx->_iccp.profile && !ctx->_user.iccp) free(ctx->_iccp.profile);
    ...
    int spng_set_sbit(Context *ctx, struct SBIT *sbit) {
      PNG_SET_CHUNK_BOILERPLATE(sbit);

      if (check_sbit(sbit, &ctx->_ihdr)) return 1;
    ...
    int spng_set_srgb(Context *ctx, uint8_t rendering_intent) {
      ...
      ctx->_srgb_rendering_intent = rendering_intent;
    ...
    int spng_set_text(Context *ctx, struct TEXT *text, uint32_t n_text) {
      if (!n_text) return 1;
      PNG_SET_CHUNK_BOILERPLATE(text);
      ...
      for (i = 0; i < n_text; i++) {
        ...
        if (text[i].type == PNG_TEXT) {
          if (ctx->_strict && check_png_text(text[i].text, text[i].length)) return 1;
          ...
          if (text[i].language_tag == NULL) return PNG_EITXT_LANG_TAG;
          if (text[i].translated_keyword == NULL) return PNG_EITXT_TRANSLATED_KEY;
        } else return 1;

      }

      struct spng_text2 *text_list = calloc(sizeof(struct spng_text2), n_text);

      if (!text_list) return PNG_EMEM;

      if (ctx->_text_list != NULL) {
        for (i = 0; i < ctx->_n_text; i++) {
          if (ctx->_user.text) break;

          free(ctx->_text_list[i].keyword);
          if (ctx->_text_list[i].compression_flag) free(ctx->_text_list[i].text);
        }
        free(ctx->_text_list);
      }
    ...
    int spng_set_bkgd(Context *ctx, struct BKGD *bkgd) {
      PNG_SET_CHUNK_BOILERPLATE(bkgd);

      if (!ctx->_stored.ihdr) return 1;

      if (ctx->_ihdr.color_type == 0 || ctx->_ihdr.color_type == 4) {
        ctx->_bkgd.gray = bkgd->gray;
      } else if (ctx->_ihdr.color_type == 2 || ctx->_ihdr.color_type == 6) {
        ctx->_bkgd.red = bkgd->red;
        ctx->_bkgd.green = bkgd->green;
        ctx->_bkgd.blue = bkgd->blue;
      } else if (ctx->_ihdr.color_type == 3) {
        if (!ctx->_stored.plte) return PNG_EBKGD_NO_PLTE;
        if (bkgd->plte_index >= ctx->_plte.n_entries) return PNG_EBKGD_PLTE_IDX;

        ctx->_bkgd.plte_index = bkgd->plte_index;
      }
    ...
    int spng_set_hist(Context *ctx, struct HIST *hist) {
      PNG_SET_CHUNK_BOILERPLATE(hist);

      if (!ctx->_stored.plte) return PNG_EHIST_NO_PLTE;
    ...
    int spng_set_phys(Context *ctx, struct PHYS *phys) {
      PNG_SET_CHUNK_BOILERPLATE(phys);
    ...
    int spng_set_splt(Context *ctx, struct SPLT *splt, uint32_t n_splt) {
      if (!n_splt) return 1;
      PNG_SET_CHUNK_BOILERPLATE(splt);
      ...
      if (ctx->_stored.splt && !ctx->_user.splt) {
        for (i = 0; i < ctx->_n_splt; i++) {
          if (ctx->_splt_list[i].entries != NULL) free(ctx->_splt_list[i].entries);
        }
        free(ctx->_splt_list);
      }
    ...
    int spng_set_time(Context *ctx, struct Time *time) {
      PNG_SET_CHUNK_BOILERPLATE(time);
    ...
    int spng_set_unknown_chunks(Context *ctx, struct UnknownChunk *chunks, uint32_t n_chunks) {
      ...
      PNG_SET_CHUNK_BOILERPLATE(chunks);

      uint32_t i;
      for (i = 0; i < n_chunks; i++) {
        if (chunks[i].length > spng_u32max) return PNG_ECHUNK_STDLEN;
      ...
      if (ctx->_stored.unknown && !ctx->_user.unknown) {
        for (i = 0; i < ctx->_n_chunks; i++) {
          free(ctx->_chunk_list[i].data);
        }
        free(ctx->_chunk_list);
      }

      ctx->_chunk_list = chunks;
      ctx->_n_chunks = n_chunks;

      ctx->_stored.unknown = 1;
      ctx->_user.unknown = 1;
    ...
    int spng_set_offs(Context *ctx, struct Offs *offs) {
      PNG_SET_CHUNK_BOILERPLATE(offs);
    ...
    int spng_set_exif(Context *ctx, struct Exif *exif) {
      PNG_SET_CHUNK_BOILERPLATE(exif);

      if (check_exif(exif)) return PNG_EEXIF;

      if (ctx->_exif.data != NULL && !ctx->_user.exif) free(ctx->_exif.data);
    ...
    static const char* spng_strerror(int err) {
      switch (err) {
        case PNG_IO_EOF:
