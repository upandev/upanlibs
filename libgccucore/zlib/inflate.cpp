...
static int inflateStateCheck(z_stream* strm)
{
    ...
    if (state == nullptr || state->strm != strm ||
 ...
 */
static int updatewindow(z_stream* strm, const byte* end, uint32_t copy) {
  struct inflate_state  *state;
  unsigned dist;
...
int inflateResetKeep(z_stream* strm)
{
    struct inflate_state  *state;
...
int inflateReset2(z_stream* strm, int windowBits)
{
    ...
    if (state->window != nullptr && state->wbits != (unsigned)windowBits) {
...
int inflateInit2(z_stream* strm, int windowBits, const char* version, int stream_size) {
    int ret;
    struct inflate_state  *state;
...
int inflateInit2(z_stream* strm, const char* version, int stream_size) {
  return inflateInit2(strm, DEF_WBITS, version, stream_size);
}

int inflateInit2(z_stream* strm, int windowBits) {
  return inflateInit2(strm, windowBits, ZLIB_VERSION, sizeof(z_stream));
}
...
int inflate(z_stream* strm, int flush)
{
    ...
    if (inflateStateCheck(strm) || strm->next_out == nullptr ||
...
#ifdef INFLATE_ALLOW_INVALID_DISTANCE_TOO_ARRR
...
int inflateEnd(z_stream* strm)
{
    ...
    if (inflateStateCheck(strm))
        return Z_STREAM_ERROR;
    state = (struct inflate_state  *)strm->state;
    if (state->window != nullptr) strm->zfree(strm->opaque, state->window);
    strm->zfree(strm->opaque, strm->state);
    strm->state = nullptr;
    return Z_OK;
}

int inflateGetDictionary(z_stream* strm, byte* dictionary, uint32_t* dictLength)
{
    struct inflate_state  *state;
...
int inflateSetDictionary(z_stream* strm, const byte* dictionary, uint32_t dictLength)
{
    struct inflate_state  *state;
 ...
 */
static unsigned syncsearch(uint32_t* have, const byte* buf, uint32_t len)
{
    unsigned got;
    unsigned next;
 ...
 */
int inflateSyncPoint(z_stream* strm)
{
    struct inflate_state  *state;
...
int inflateCopy(z_stream* dest, z_stream* source)
{
    ...
    /* check input */
    if (inflateStateCheck(source) || dest == nullptr)
        return Z_STREAM_ERROR;
    state = (struct inflate_state  *)source->state;

    /* allocate space */
    copy = (struct inflate_state  *)source->zalloc(source->opaque, 1, sizeof(struct inflate_state));
    if (copy == nullptr) return Z_MEM_ERROR;
    window = nullptr;
    if (state->window != nullptr) {
        window = (unsigned char  *)source->zalloc(source->opaque, 1U << state->wbits, sizeof(unsigned char));
        if (window == nullptr) {
          source->zfree(source->opaque, copy);
          return Z_MEM_ERROR;
        }
    }
...
int inflateValidate(z_stream* strm, int check)
{
    struct inflate_state  *state;
...
unsigned long inflateCodesUsed(z_stream* strm)
{
    struct inflate_state  *state;
