...
static unsigned read_buf(z_stream* strm, byte *buf, unsigned size);
...
/* ========================================================================= */
int deflateInit2(z_stream* strm, int level, int method, int windowBits, int memLevel, int strategy, const char* version, int stream_size) {
    deflate_state *s;
    int wrap = 1;
    ...
    s->window = (byte *)strm->zalloc(strm->opaque, s->w_size, 2*sizeof(byte));
    ...
    overlay = (uint16_t*)strm->zalloc(strm->opaque, s->lit_bufsize, sizeof(uint16_t)+2);
...
int deflateInit2(z_stream* strm, int level, int method, int windowBits, int memLevel, int strategy) {
  return deflateInit2(strm, level, method, windowBits, memLevel, strategy, ZLIB_VERSION, sizeof(z_stream));
}

int deflateInit2(z_stream* strm, int level, const char* version, int stream_size)
{
    return deflateInit2(strm, level, Z_DEFLATED, MAX_WBITS, DEF_MEM_LEVEL, Z_DEFAULT_STRATEGY, version, stream_size);
}

int deflateInit2(z_stream* strm, int level)
{
    return deflateInit2(strm, level, ZLIB_VERSION, sizeof(z_stream));
}
...
/* ========================================================================= */
int deflateSetDictionary (z_stream* strm, const byte* dictionary, uint32_t dictLength)
{
    deflate_state *s;
    uint32_t str, n;
    int wrap;
    ...
    if (deflateStateCheck(strm) || dictionary == nullptr)
        return Z_STREAM_ERROR;
    s = strm->state;
    wrap = s->wrap;
...
/* ========================================================================= */
int deflateGetDictionary (z_stream* strm, byte* dictionary, uint32_t* dictLength)
{
    deflate_state *s;
    uint32_t len;
...
/* ========================================================================= */
int deflatePending (z_stream* strm, unsigned* pending, int* bits)
{
...
int deflateEnd (z_stream* strm)
{
    ...
    if (strm->state->pending_buf) strm->zfree(strm->opaque, strm->state->pending_buf);
    if (strm->state->head) strm->zfree(strm->opaque, strm->state->head);
    if (strm->state->prev) strm->zfree(strm->opaque, strm->state->prev);
    if (strm->state->window) strm->zfree(strm->opaque, strm->state->window);
...
int deflateCopy (z_stream* dest, z_stream* source)
{
    ...
    if (deflateStateCheck(source) || dest == nullptr) {
        return Z_STREAM_ERROR;
    }
    ...
    ds->window = (byte *) dest->zalloc(dest->opaque, ds->w_size, 2*sizeof(byte));
    ds->prev   = (Posf *)  dest->zalloc(dest->opaque, ds->w_size, sizeof(Pos));
    ds->head   = (Posf *)  dest->zalloc(dest->opaque, ds->hash_size, sizeof(Pos));
    overlay = (uint16_t *) dest->zalloc(dest->opaque, ds->lit_bufsize, sizeof(uint16_t)+2);
    ds->pending_buf = (byte *) overlay;
 ...
 */
static unsigned read_buf(z_stream* strm, byte* buf, uint32_t size)
{
...
static uint32_t longest_match(deflate_state* s, IPos cur_match)
{
     ...
     */
    assert_f(s->hash_bits >= 8 && MAX_MATCH == 258, "Code too clever");
    ...
    assert_f((unsigned long)s->strstart <= s->window_size-MIN_LOOKAHEAD, "need lookahead");

    do {
        assert_f(cur_match < s->strstart, "no future");
         ...
         */
        assert_f(scan[2] == match[2], "scan[2]?");
        scan++, match++;
        do {
        ...
        assert_f(scan <= s->window+(unsigned)(s->window_size-1), "wild scan");
        if (*scan == *match) scan++;
...
static void fill_window(deflate_state *s)
{
    ...
    assert_f(s->lookahead < MIN_LOOKAHEAD, "already enough lookahead");
    ...
    assert_f((unsigned long)s->strstart <= s->window_size - MIN_LOOKAHEAD,
...
static block_state deflate_slow(deflate_state* s, int flush)
{
    ...
    assert_f (flush != Z_NO_FLUSH, "no flush?");
...
static block_state deflate_rle(deflate_state* s, int flush)
{
    ...
    for (;;) {
        ...
        if (s->lookahead >= MIN_MATCH && s->strstart > 0) {
            ...
            assert_f(scan <= s->window+(uint32_t)(s->window_size-1), "wild scan");
        }
